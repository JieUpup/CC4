...
print(obs.keys())  # Output: dict_keys(['blue_agent_0', 'blue_agent_1', 'blue_agent_2', 'blue_agent_3', 'blue_agent_4'])
    ...
    for host, details in agent_obs.items():
        ...
        if isinstance(details, dict):
            ...
            for interface in interfaces:
                print(f"Interface: {interface['interface_name']}, IP: {interface['ip_address']}, Subnet: {interface['Subnet']}")
            
            # Print session details
            sessions = details.get('Sessions', [])
            for session in sessions:
                print(f"Session ID: {session['session_id']}, User: {session['username']}, PID: {session['PID']}, Agent: {session['agent']}")
